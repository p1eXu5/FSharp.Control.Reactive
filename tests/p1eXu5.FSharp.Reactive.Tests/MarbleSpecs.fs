module p1eXu5.FSharp.Reactive.Tests.MarbleSpecs

open p1eXu5.FSharp.Reactive
open p1eXu5.FSharp.Reactive.Testing
open p1eXu5.FSharp.Reactive.Testing.Marbles
open NUnit.Framework
open FsCheck

let test f =
    Check.QuickThrowOnFailure <| fun () ->
    TestScheduler.usage <| fun sch -> f sch
...
let ``cold: marble should parse concat`` () =
    ...
    |> Marbles.expectMessages sch e
    TestObservable.subscriptions a |> Marbles.expectSubscriptions [sa]
    TestObservable.subscriptions b |> Marbles.expectSubscriptions [sb]
